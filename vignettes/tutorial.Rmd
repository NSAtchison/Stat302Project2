...
tutorial_comp_mat <- matrix(data = NA, nrow = nrow(my_penguins), ncol = 2)
for(i in 1:10) {
  curr_k_nn <- i
  incorrect <- 0
  tutorial_my_knn <- my_knn_cv(train = my_penguins[,3:6], cl = my_penguins$species, k_nn = curr_k_nn, k_cv = 5)
  tutorial_comp_mat[,1] <- as.vector(my_penguins$species)
  #tutorial_comp_mat[,2] <- as.vector(tutorial_my_knn$Class)
  #for(i in 1:length(my_penguins$species)) {
    #if(tutorial_comp_mat[i,1] != tutorial_comp_mat[i,2]) {
     # incorrect <- incorrect + 1
    #}
  #}
  #tutorial_table[i,1] <- incorrect / length(my_penguins$species)
  #tutorial_table[i,2] <- tutorial_my_knn$CV_error
}
